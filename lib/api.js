...
var request = require("request").defaults({rejectUnauthorized:false, secureProtocol: 'TLSv1_method', strictSSL: false})
...
API.prototype.pull = function(sn, retryno) {
  ...
  this.sn = request({
    ...
    if(!resp) return console.log('Mega server req failed, resp is undefined')
    if (resp.w) {
      self.wait(resp.w, sn)
    }
    else if (resp.sn) {
